                    ...
                    <div class="bg-gray-900 rounded-lg border border-white/10 overflow-hidden">
                        ...
                        <div class="divide-y divide-white/10">
                            @forelse($category->lessons as $lesson)
                                <a href="{{ route('lms.lesson', $lesson) }}" class="block px-6 py-4 hover:bg-gray-800/50 transition group">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <h3 class="text-lg font-semibold text-white group-hover:text-blue-400 transition">{{ $lesson->title }}</h3>
                                        ...
                                        <div class="ml-4 flex items-center gap-2">
                                            @if($lesson->duration)
                                                <span class="px-3 py-1 bg-gray-800 text-gray-300 text-sm rounded">
                                                    {{ $lesson->duration }} min
                                                </span>
                                            @endif
                                            @if($lesson->video_url)
