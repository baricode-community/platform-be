@props(['href' => '#', 'icon' => 'link', 'active' => false])

@php
    $isActive = request()->is(ltrim(parse_url($href)['path'], '/')) || 
                request()->routeIs(basename($href)) ||
                request()->url() === $href;
@endphp

<a href="{{ $href }}"
    class="group relative flex items-center space-x-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 {{ $isActive ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800' }}">
